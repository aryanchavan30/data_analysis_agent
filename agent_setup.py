...
# ---------- Middleware #2: session_tool_router (Snekbox HTTP) ----------

@wrap_tool_call
async def session_tool_router(request: ToolCallRequest, handler):
    """Routes python_repl_ast execution to Snekbox via HTTP."""
    if request.tool_call["name"] != "python_repl_ast":
        return await handler(request)
    ...
    # Prepend setup code (imports + DataFrame reloads) since Snekbox is stateless
    setup_code = session.get_setup_code()
    ...
    try:
        ...
        output = "Execution error: code timed out (exceeded Snekbox timeout limit)."
...
middleware_stack = [
    ...
    # 10: Session-aware Snekbox routing (innermost — replaces default execution)
    session_tool_router,
]
